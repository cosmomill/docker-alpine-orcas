...
ENV ANT_CONTRIB_DOWNLOAD_URL http://downloads.sourceforge.net/project/ant-contrib/ant-contrib/$ANT_CONTRIB_VERSION/ant-contrib-$ANT_CONTRIB_VERSION-bin.tar.gz

# download and extract apache ant contrib
RUN curl -f#L $ANT_CONTRIB_DOWNLOAD_URL | bsdtar --strip-components=1 -C $ANT_HOME/lib -xf- ant-contrib/ant-contrib-1.0b3.jar
...
ONBUILD RUN bsdtar -C /usr/local/lib -xf /tmp/instantclient-basic-linux.x64-$ORACLE_INSTANTCLIENT_VERSION.zip \
	...
	&& find /usr/local/lib/instantclient_$ORACLE_INSTANTCLIENT_MAJOR -type f -exec chmod 644 {} \; \
...
# pull Orcas
RUN git clone --progress --verbose --recursive "$ORCAS_GIT_REPOSITORY" /opt/orcas/
